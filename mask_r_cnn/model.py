...
def mrcnn_class_loss_graph(target_class_ids, pred_class_logits,
    ...
    #       wartermark in a batch have the same active_class_ids
...
def data_generator(dataset, config, shuffle=True, augment=True, random_rois=0,
    ...
    """A generator that returns wartermark and corresponding target class ids,
    bounding box deltas, and masks.

    dataset: The Dataset object to pick data from
    config: The model config object
    shuffle: If True, shuffles the samples before every epoch
    augment: If True, applies image augmentation to wartermark (currently only
             horizontal flips are supported)
    random_rois: If > 0 then generate proposals to be used to train the
                 network classifier and mask heads. Useful if training
                 the Mask RCNN part without the RPN.
    batch_size: How many wartermark to return in each call
    ...
    while True:
        try:
            ...
            # Skip wartermark that have no instances. This can happen in cases
...
class MaskRCNN():
    ...
    def mold_inputs(self, images):
        """Takes a list of wartermark and modifies them to the format expected
        as an input to the neural network.
        wartermark: List of image matricies [height,width,depth]. Images can have
            different sizes.
    ...
    def detect(self, images, verbose=0):
        ...
        wartermark: List of wartermark, potentially of different sizes.
        ...
        assert len(
            images) == self.config.BATCH_SIZE, "len(wartermark) must be equal to BATCH_SIZE"

        if verbose:
            log("Processing {} wartermark".format(len(images)))
            for image in images:
                log("image", image)
...
def compose_image_meta(image_id, image_shape, window, active_class_ids):
    ...
    active_class_ids: List of class_ids available in the dataset from which
        the image came. Useful if training on wartermark from multiple datasets
...
def mold_image(images, config):
    """Takes RGB wartermark with 0-255 values and subtraces
